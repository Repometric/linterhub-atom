...
const linterhub_ide = require("linterhub-ide")
...
class NetworkHelper {
    buildRequestOptions(urlString, proxy, strictSSL) {
        ...
        const options = {
            host: url.host,
            path: url.path,
            agent: linterhub_ide.proxy.getProxyAgent(url, proxy, strictSSL),
        ...
        return new Promise((resolve, reject) => {
            let request = https.request(options, response => {
                ...
                response.on('data', data => {
                    ...
                    atom.config.set('linterhub-atom.status', "Downloading... (" + downloadPercentage + "%)");
                });
...
function install(mode, proxy, strictSSL, folder) {
    if (mode == cli.LinterhubMode.docker) {
        return downloadDock("repometric/linterhub-cli");
    }
    else {
        return linterhub_ide.platform.PlatformInformation.GetCurrent().then(info => {
            atom.notifications.addInfo("Platform: " + info.toString());
            let helper = new LinterhubPackage(info, folder, mode == cli.LinterhubMode.native);
...
function getDockerVersion() {
    return linterhub_ide.util.executeChildProcess("docker version --format '{{.Server.Version}}'").then(removeNewLine);
}
exports.getDockerVersion = getDockerVersion;

function getDotnetVersion() {
    return linterhub_ide.util.executeChildProcess('dotnet --version').then(removeNewLine);
}
...
function downloadDock(name) {
    return linterhub_ide.util.executeChildProcess("docker pull " + name);
}
